...
class KnowledgeSearchServiceTest {
    ...
    void blankQueryBrowsesKnowledgeAndPublishedContentWithoutEmbedding() {
        KnowledgeDoc doc = new KnowledgeDoc("博主简介", KnowledgeSourceType.MANUAL, null, "Java 后端开发", true);
        ContentKnowledgeSource content = new ContentKnowledgeSource(UUID.randomUUID(), "测试文章", "文章摘要", null);
        ...
        assertThat(results).extracting(KnowledgeSearchResult::title).containsExactly("博主简介", "测试文章");
        verify(embeddingService, never()).embed(any(String.class));
    }

    @Test
    void exactKeywordResultWinsWithoutVectorSearch() {
        KnowledgeDoc doc = new KnowledgeDoc("博主简介", KnowledgeSourceType.MANUAL, null, "我是 Java 后端开发者", true);
        when(knowledgeDocRepository.searchEnabled(eq("Java"), any())).thenReturn(List.of(doc));
    ...
    void keywordExcerptIncludesMatchesNearTheEndOfLongDocuments() {
        KnowledgeDoc doc = new KnowledgeDoc(
                "长文档", KnowledgeSourceType.MANUAL, null, "前文".repeat(700) + "目标关键词" + "后文".repeat(100), true);
        when(knowledgeDocRepository.searchEnabled(eq("目标关键词"), any())).thenReturn(List.of(doc));
    ...
    void filtersLowSimilarityVectorCandidates() {
        ...
        when(knowledgeChunkRepository.findSimilarChunks(any(String.class), anyInt()))
                .thenReturn(List.<Object[]>of(new Object[] {UUID.randomUUID(), docId, null, 0, "无关片段", null, 0.59}));
    ...
    void returnsRelevantVectorCandidateWithSourceTitle() {
        ...
        when(knowledgeChunkRepository.findSimilarChunks(any(String.class), eq(25)))
                .thenReturn(List.<Object[]>of(
                        new Object[] {UUID.randomUUID(), doc.getId(), null, 0, "我是 Java 后端开发者", null, 0.66}));
    ...
    void deduplicatesVectorCandidatesFromTheSameSource() {
        ...
        when(knowledgeChunkRepository.findSimilarChunks(any(String.class), eq(25)))
                .thenReturn(List.of(
                        new Object[] {UUID.randomUUID(), doc.getId(), null, 0, "第一段", null, 0.78},
                        new Object[] {UUID.randomUUID(), doc.getId(), null, 1, "第二段", null, 0.71}));
        when(knowledgeDocRepository.findAllById(List.of(doc.getId()))).thenReturn(List.of(doc));
