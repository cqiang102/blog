...
public class AiConfig {
    ...
    private static final String SYSTEM_PROMPT_TEMPLATE = """
            ...
            角色和代词规则：
            1. “博主”“站长”“管理员”“AI助手”“你”通常指沐凉个人博客的主人或代表他的本站助手。
            ...
            5. 当用户问“你是谁/博主是谁/管理员是谁”时，搜索知识库并按博主资料回答。
            6. 不确定代词指向时，先按以上规则判断；仍有歧义时用一句话说明你按哪个角色理解。
    ...
    public static String systemPrompt(AuthenticatedUser currentUser) {
        return SYSTEM_PROMPT_TEMPLATE
                .replace("{{currentUserId}}", currentUser.id().toString())
                .replace("{{currentUserNickname}}", currentUser.nickname())
                .replace("{{currentUserEmail}}", currentUser.email())
                .replace("{{currentUserRole}}", currentUser.role().name());
    }

    private static String defaultSystemPrompt() {
        return SYSTEM_PROMPT_TEMPLATE
                .replace("{{currentUserId}}", "未提供")
                .replace("{{currentUserNickname}}", "未提供")
                .replace("{{currentUserEmail}}", "未提供")
                .replace("{{currentUserRole}}", "未提供");
    }
