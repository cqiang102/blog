...
public interface AiChatMessageRepository extends JpaRepository<AiChatMessage, UUID> {
    ...
    List<AiChatMessage> findTop20BySessionIdAndDeletedFalseOrderByCreatedAtDescIdDesc(UUID sessionId);
    ...
    /**
     * 分页获取指定会话的未删除消息，按创建时间正序排列。
     *
     * @param sessionId 会话 ID
     * @param pageable  分页参数
     * @return 分页未删除消息列表
     */
    Page<AiChatMessage> findBySessionIdAndDeletedFalseOrderByCreatedAtAsc(UUID sessionId, Pageable pageable);
     ...
     * @return 最后一条未删除消息
     */
    Optional<AiChatMessage> findFirstBySessionIdAndDeletedFalseOrderByCreatedAtDesc(UUID sessionId);
}
